...
class UserDirectoryController(
    ...
    @GetMapping
    @Transactional
    fun userPage(@GetSession user: Session, @RequestParam(required = false, defaultValue = "/") path: String): ModelAndView {
    ...
    @ResponseBody
    @Transactional
    fun uploadFile(@GetSession user: Session, @RequestParam("path") path: String, @RequestParam("file") file: MultipartFile): ResponseEntity<*> {
